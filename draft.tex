...
Consider a Metropolis-Hastings transition where we condition on $\thetav$ and propose changing the random seeds.  It is straightforward to show that this leaves the target distribution invariant.  Let $\pi(\omega_s)$ be a uniform prior over random seeds.  We propose $\omega_s^{'} \sim q(\omega_s^{'} | \omega_s) = \pi(\omega_s)$ (independent of the current seed).  Pseudo-data is simulated deterministically $\x^{'(s)} = f( \thetav, \omega_s^{'})$.  Our proposal distribution has probability $\pi(x^{'(s)} | \thetav, \omega_s^{'}) = \delta( x^{'(s)} - f( \thetav, \omega_s^{'}))$.  The acceptance probability ratio reduces to a ratio of likelihoods [TODO]:
% \begin{equation}
...
\caption{\small{Trajectories of two parameters over the last 1000 time-steps.  {\bf Top:} SGLD and {\bf Bottom}: SG-Thermostats.  Relative to SL-MCMC (not shown), the Hamiltonian dynamics clearly show persistent $\thetav$ trajectories.}}
